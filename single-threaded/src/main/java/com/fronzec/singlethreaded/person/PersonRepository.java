...
public interface PersonRepository extends JpaRepository<PersonsEntity, Long> {
  List<PersonsEntity> findByIdGreaterThanAndProcessedIsFalseOrderByIdAsc(
    Long id,
    Pageable pageable
  );
  ...
  @Modifying
  @Query(
    value = "update persons set processed = true, updated_at = CURRENT_TIMESTAMP where id in (:ids)",
    nativeQuery = true
  )
