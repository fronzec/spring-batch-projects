...
public class BatchConfiguration {
  ...
  public BatchConfiguration(JobBuilderFactory jobBuilderFactory, StepBuilderFactory stepBuilderFactory, JobRepository jobRepository) {
    ...
    String anigvariables = "asdfasfdsf";
    String anigvariables2 = "asdfasfdsf";
    String anigvariables3 = "asdfasfdsf";
    bigString =
            String.format("this is a big string with templated value  %s -Zsdfdsfasdfdsafdfdfdsaafdsafdsafdsfdsfdsa sfdsfdsf ", 1, 2, 3, 4,
                    5, 65, 6, anigvariables, anigvariables2, anigvariables3);
  }
  ...
  public JdbcBatchItemWriter<CustomerOutput> writerCustomer(DataSource dataSource) {
    return new JdbcBatchItemWriterBuilder<CustomerOutput>().itemSqlParameterSourceProvider(
                    new BeanPropertyItemSqlParameterSourceProvider<>())
            .sql("INSERT INTO customer (firstName, lastName, fullName, email, profession) "
                    + "VALUES (:firstName, :lastName, :fullName, :email, :profession)")
            .dataSource(dataSource)
            .build();
  }
  ...
  public Job importUserJob(JobCompletionNotificationListener listener, Step step1) {
    ...
    return jobBuilderFactory.get("myImportUserJob")// JobName: The jobname could be different from bean name but is common to have the
            // same value
  ...
  public Job importCustomerJob(JobCompletionNotificationListener listener, Step stepCustomer2) {
    return jobBuilderFactory.get("importCustomerJob")// JobName: The jobname could be different from bean name but is common to have
            // the same value
