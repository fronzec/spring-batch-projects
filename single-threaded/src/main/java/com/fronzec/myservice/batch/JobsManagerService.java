...
import com.fronzec.myservice.web.SingleJobDataRequest;
...
public class JobsManagerService {
  ...
  public void init() {
    ...
    // TODO: 19/02/2022 refactor default params to add boolean that allow indicate if param is used as identifying a job instance
  ...
  public HashMap<String, String> launchAllNonAutoDetectedJobsSync(LocalDate execDate, final int runningNumber) {
    ...
    jobs.forEach((key, defaultJobParams) -> {
      try {
        ...
        try {
          logger.info("Running job: {} with params -> {}", theJob, defaultJobParams);
